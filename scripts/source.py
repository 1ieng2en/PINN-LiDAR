...
class Source(ABC):
    ...
    def __init__(self, **kwargs):
        self.params = {}
        self._f = kwargs.get('freq', 1000)  # 先初始化_f
    ...
    def _update_derived_params(self):
        ...
        self.params['c'] = self.params.get('c', 343)         # Default speed of sound if not set
        self.params['omega'] = 2 * np.pi * self._f
        self.params['k'] = self.params['omega'] / self.params['c']
        
    def update_params(self, **kwargs):
        if 'freq' in kwargs:
            self.freq = kwargs['freq']  # 更新_f
        if 'freq' in self.params:
            self.params['freq']  = self.freq
    ...
    @abstractmethod
    def source_mesh(self, **kwargs):
        """Generate source mesh points and distances.
        
        Returns:
            tuple: (points, distances)
                points: np.ndarray of shape (N, 3) for point coordinates
                distances: np.ndarray of shape (N,) for distances from origin
        """
        pass
...
class SphericalSource(Source):
    ...
    def torch_spherical_jn(self, n, x):
        x = x.cpu().numpy()  # 先转回 NumPy
        y = spherical_jn(n, x)
        return torch.from_numpy(y).to(x.device)

    def torch_spherical_yn(self, n, x):
        x = x.cpu().numpy()  # 先转回 NumPy
        y = spherical_yn(n, x)
        return torch.from_numpy(y).to(x.device)
    

    def compute_pressure(self, obs, **kwargs):
        """Dispatch method to compute pressure based on input type."""
        if isinstance(obs, np.ndarray):
            return self.compute_pressure_numpy(obs, **kwargs)
        elif isinstance(obs, torch.Tensor):
            return self.compute_pressure_torch(obs, **kwargs)
        else:
            raise TypeError("Unsupported type for obs. Expected numpy.ndarray or torch.Tensor.")

    def compute_pressure_numpy(self, obs, **kwargs):
        """Compute pressure using NumPy."""
        # 原有的 NumPy 实现
        params = {**self.params, **kwargs}
        omega = params['omega']
        k = params['k']
        amp = params.get('amp', 1.0)
        loc = np.array(params.get('loc', [0, 0, 0]))
        radius = params.get('radius', 0.05)
        max_order = params.get('max_order', 10)
        t = params.get('t', 0)
        ...
        for n in range(max_order + 1):
            jn_ka = self.torch_spherical_jn(n, k * radius)
            hn_kr = self.torch_spherical_jn(n, k * r_obs) - 1j * self.torch_spherical_yn(n, k * r_obs)
    ...
    def compute_pressure_torch(self, obs, **kwargs):
        """Compute pressure using PyTorch."""
        # GPU版本的实现
        params = {**self.params, **kwargs}
        omega = params['omega']
        k = params['k']
        amp = params.get('amp', 1.0)
        loc = torch.tensor(params.get('loc', [0, 0, 0]), device=obs.device)
        radius = params.get('radius', 0.05)
        max_order = params.get('max_order', 10)
        t = params.get('t', 0)

        obs_rel = obs - loc
        r_obs = torch.norm(obs_rel, dim=1)
        theta_obs = torch.acos(torch.clamp(obs_rel[:, 2] / r_obs, -1.0, 1.0))
        self.theta_obs = theta_obs
        sp = torch.zeros_like(r_obs, dtype=torch.complex64)
        ...
        for n in range(max_order + 1):
            ...
            An = amp * (2 * n + 1) * jn_ka
            Pn_cos_theta = lpmv(0, n, torch.cos(theta_obs))

            # 确保 Pn_cos_theta 是 torch.Tensor
            if not isinstance(Pn_cos_theta, torch.Tensor):
                Pn_cos_theta = torch.tensor(Pn_cos_theta, device=obs.device)

            sp += An * hn_kr * Pn_cos_theta

        if t is not None:
            sp *= torch.exp(-1j * omega * t)

        sp /= r_obs
        return sp.cpu().numpy() 

    def source_mesh(self, **kwargs):
        """Generate spherical source mesh."""
        # Create spherical mesh using spherical coordinates
        res = self.params.get("res", 40)
        radius = self.params.get("radius", "no radius")
        ...
        # Add location offset if provided
        if 'loc' in kwargs:
            points += np.array(kwargs['loc'])
...
class DipoleSource(Source):
    ...
    def source_mesh(self, res=30, sep=0.1, amp=1.0, t=0, **kwargs):
        """Generate dipole source mesh (two points)."""
        # Create two points separated by sep distance
        x = np.array([-sep/2, sep/2])
        y = np.zeros(2)
        z = np.zeros(2)
...
class CylinderSource(Source):
    ...
    def compute_pressure(self, obs, **kwargs):
        ...
        k = params['k']
        amp = params.get('amp', 1.0)
        radius = params.get('radius', [0, 0, 0])
        t = params.get('t', 0)
    ...
    def source_mesh(self, res=30, radius=0.05, length=1.0, amp=1.0, t=0, **kwargs):
        """Generate cylindrical source mesh."""
        # Create cylinder mesh using cylindrical coordinates
        theta = np.linspace(0, 2*np.pi, res)
        h = np.linspace(-length/2, length/2, res)
...
def create_source(source_type: str, **kwargs) -> Union[SphericalSource, DipoleSource, CylinderSource]:
